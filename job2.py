...
def parse_one_page(html):
    ...
    for item in items:
        ...
        salary_avarage = 0
        temp = item[3]
        # print('工闪', temp.endswith('以下'))
        if temp != '面议' and (not temp.endswith('以下')):
            idx = temp.find('-')
            # print('工资', temp)
            # 求平均工资
...
def main(city, keyword, region, pages):
    ...
    for i in range(pages):
        ...
        for item in items:
            ...
            if job_detail['requirement'].find('双休') > -1:
               print('工作详情', job_detail)
               companyList.append(item.get('job_url'))
